...
if(d.client_secret){setClientSecret(d.client_secret);setMontantMax(analyse?parseInt((analyse.prix_max||"300").replace(/[^0-9]/g,""))||300:300);setPaymentReady(true);setStep("cb");}
...
                      </div>
                    ))}
                    {l.paiement_statut==="en_attente_validation"&&<div style={{marginTop:14,paddingTop:14,borderTop:"0.5px solid rgba(255,255,255,0.06)",background:"rgba(255,111,0,0.08)",padding:"12px",borderRadius:10}}><div style={{fontSize:11,fontWeight:700,color:"#FF6F00",marginBottom:8}}>ARTISAN A TERMINE</div><div style={{fontSize:13,color:"rgba(255,255,255,0.8)",marginBottom:12}}>{l.artisan_nom||"L artisan"} propose : <span style={{fontWeight:700,color:"#FF6F00"}}>{l.prix_final}EUR</span></div><div style={{display:"flex",gap:8}}><button onClick={async e=>{e.stopPropagation();const AK="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpcHF0cWV6bnR6Y214d2lhcWR6Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MDA3OTkxMCwiZXhwIjoyMDk1NjU1OTEwfQ.NJxvcp7MJEGbpNmvjkwDGc4CJCswcoLZdGUSw0EDisU";await fetch("https://bipqtqezntzcmxwiaqdz.supabase.co/rest/v1/leads?id=eq."+l.id,{method:"PATCH",headers:{"Content-Type":"application/json","apikey":AK,"Authorization":"Bearer "+AK},body:JSON.stringify({paiement_statut:"paye"})});fetch("/api/capture-payment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({payment_intent_id:l.payment_intent_id,amount_final:l.prix_final})}).then(r=>r.json()).then(d=>ctx.notify(d.success?"Paiement de "+l.prix_final+"EUR confirme !":"Erreur paiement","err"));}} style={{flex:1,padding:"10px",background:"#22c55e",border:"none",borderRadius:10,color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer"}}>✓ Valider et payer</button><button onClick={e=>{e.stopPropagation();const AK="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpcHF0cWV6bnR6Y214d2lhcWR6Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MDA3OTkxMCwiZXhwIjoyMDk1NjU1OTEwfQ.NJxvcp7MJEGbpNmvjkwDGc4CJCswcoLZdGUSw0EDisU";fetch("https://bipqtqezntzcmxwiaqdz.supabase.co/rest/v1/leads?id=eq."+l.id,{method:"PATCH",headers:{"Content-Type":"application/json","apikey":AK,"Authorization":"Bearer "+AK},body:JSON.stringify({paiement_statut:"pre_autorise"})});ctx.notify("Prix refuse");}} style={{flex:1,padding:"10px",background:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:10,color:"#ef4444",fontWeight:700,fontSize:13,cursor:"pointer"}}>✕ Refuser</button></div></div>}
                    {l.paiement_statut==="paye"&&<div style={{marginTop:12,padding:"12px",background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:10}}><div style={{fontSize:13,fontWeight:700,color:"#22c55e",marginBottom:8}}>✅ Intervention payée — {l.prix_final}EUR</div><button onClick={e=>{e.stopPropagation();const showInvoice=()=>{const doc=new window.jspdf.jsPDF();const prix=parseFloat(l.prix_final)||0;const tva=0;const ht=prix;doc.setFillColor(255,111,0);doc.rect(0,0,210,35,"F");doc.setFont("helvetica","bold");doc.setFontSize(26);doc.setTextColor(255,255,255);doc.text("Click&fix",20,22);doc.setFontSize(11);doc.setFont("helvetica","normal");doc.text("Plateforme de services a domicile",20,30);doc.setFontSize(20);doc.setFont("helvetica","bold");doc.setTextColor(0,0,0);doc.text("FACTURE",150,22,{align:"right"});doc.setFontSize(11);doc.setFont("helvetica","normal");doc.setTextColor(100,100,100);doc.text("N° CF-"+new Date().getFullYear()+"-"+String(l.id).padStart(4,"0"),150,30,{align:"right"});doc.setFontSize(10);doc.setTextColor(80,80,80);doc.text("Date: "+new Date(l.created_at).toLocaleDateString("fr-FR"),20,48);doc.text("Lieu: France",20,56);doc.setDrawColor(220,220,220);doc.line(20,62,190,62);doc.setFillColor(245,245,247);doc.roundedRect(20,66,80,35,2,2,"F");doc.roundedRect(110,66,80,35,2,2,"F");doc.setFont("helvetica","bold");doc.setFontSize(9);doc.setTextColor(150,150,150);doc.text("CLIENT",25,73);doc.text("ARTISAN",115,73);doc.setFont("helvetica","bold");doc.setFontSize(11);doc.setTextColor(0,0,0);doc.text(l.client_nom||"Client",25,81);doc.text(l.artisan_nom||"Artisan",115,81);doc.setFont("helvetica","normal");doc.setFontSize(9);doc.setTextColor(100,100,100);doc.text(l.client_email||"",25,88);doc.text("Click&fix Partner",115,88);doc.text(l.client_tel||"",25,95);doc.setDrawColor(220,220,220);doc.line(20,108,190,108);doc.setFillColor(255,111,0);doc.rect(20,112,170,10,"F");doc.setFont("helvetica","bold");doc.setFontSize(9);doc.setTextColor(255,255,255);doc.text("DESCRIPTION",25,119);doc.text("QTE",130,119);doc.text("MONTANT",170,119,{align:"right"});doc.setFillColor(252,252,252);doc.rect(20,122,170,20,"F");doc.setFont("helvetica","bold");doc.setFontSize(10);doc.setTextColor(0,0,0);const trav=l.travaux||"Intervention";doc.text(trav.charAt(0).toUpperCase()+trav.slice(1),25,132);doc.setFont("helvetica","normal");doc.setFontSize(9);doc.setTextColor(100,100,100);if(l.details){const det=doc.splitTextToSize(l.details,90);doc.text(det,25,139);}doc.setFont("helvetica","normal");doc.setFontSize(10);doc.setTextColor(0,0,0);doc.text("1",135,132);doc.text(prix.toFixed(2)+" EUR",170,132,{align:"right"});doc.setDrawColor(220,220,220);doc.line(20,145,190,145);doc.setFont("helvetica","normal");doc.setFontSize(10);doc.setTextColor(80,80,80);doc.text("Sous-total HT:",130,155);doc.text(ht.toFixed(2)+" EUR",190,155,{align:"right"});doc.setFont("helvetica","italic");doc.setFontSize(9);doc.setTextColor(120,120,120);doc.text("TVA non applicable - Art. 293 B du CGI",130,163);doc.text("0.00 EUR",190,163,{align:"right"});doc.setFillColor(30,30,30);doc.roundedRect(110,168,80,16,2,2,"F");doc.setFont("helvetica","bold");doc.setFontSize(12);doc.setTextColor(255,255,255);doc.text("Total TTC:",120,179);doc.text(prix.toFixed(2)+" EUR",188,179,{align:"right"});doc.setFont("helvetica","normal");doc.setFontSize(8);doc.setTextColor(150,150,150);doc.text("Paiement effectue via Stripe - Reference: "+(l.payment_intent_id||"N/A"),20,200);doc.setDrawColor(255,111,0);doc.setLineWidth(0.3);doc.line(20,265,190,265);doc.setFontSize(8);doc.text("Click&fix — contact@click-fix.fr — www.click-fix.fr",105,270,{align:"center"});doc.text("Plateforme de mise en relation entre particuliers et artisans",105,276,{align:"center"});const url=doc.output("bloburl");const overlay=document.createElement("div");overlay.style.cssText="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;";const btn=document.createElement("button");btn.textContent="✕ Fermer";btn.style.cssText="margin-bottom:12px;padding:8px 20px;background:#fff;border:none;border-radius:8px;font-weight:700;cursor:pointer;font-size:14px;";btn.onclick=()=>document.body.removeChild(overlay);const iframe=document.createElement("iframe");iframe.src=url;iframe.style.cssText="width:90%;max-width:700px;height:80vh;border:none;border-radius:12px;";overlay.appendChild(btn);overlay.appendChild(iframe);document.body.appendChild(overlay);};if(window.jspdf){showInvoice();}else{const s=document.createElement("script");s.src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js";s.onload=showInvoice;document.head.appendChild(s);}}} style={{width:"100%",padding:"10px",background:"rgba(34,197,94,0.15)",border:"1px solid rgba(34,197,94,0.3)",borderRadius:10,color:"#22c55e",fontWeight:700,fontSize:13,cursor:"pointer"}}>📄 Voir la facture</button></div>}
                  </div>
                )}
              </div>
            ))}
...
                      {[["Spécialité",l.precision],["Détails",l.details],["Adresse",l.adresse],["Durée estimée",l.analyse_ia&&l.analyse_ia.duree],["Estimation",l.analyse_ia&&l.analyse_ia.prix_min?l.analyse_ia.prix_min+" - "+l.analyse_ia.prix_max:null]].map(([k,v])=>v&&(<div key={k} style={{display:"flex",gap:12}}><span style={{fontSize:11,color:"rgba(255,255,255,0.25)",minWidth:80,flexShrink:0}}>{k}</span><span style={{fontSize:12,color:"rgba(255,255,255,0.75)"}}>{v}</span></div>))}
                    </div>
...
                    {l.assigned_to&&<ArtisanInfo id={l.assigned_to}/>}
                    {l.photo&&<a href={l.photo} target="_blank" rel="noopener noreferrer" style={{display:"block",marginTop:12,borderRadius:12,overflow:"hidden"}}><img src={l.photo} alt="Photo" style={{width:"100%",maxHeight:180,objectFit:"cover",borderRadius:12}}/><div style={{fontSize:11,color:"rgba(255,255,255,0.4)",textAlign:"center",marginTop:4}}>Voir la photo en grand →</div></a>}{l.analyse_ia&&l.analyse_ia.diagnostic&&<div style={{marginTop:12,padding:"12px",background:"rgba(34,197,94,0.05)",border:"1px solid rgba(34,197,94,0.15)",borderRadius:12}}><div style={{fontSize:10,fontWeight:700,color:"#22c55e",letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>Diagnostic IA</div><div style={{fontSize:13,color:"rgba(255,255,255,0.8)",marginBottom:4}}>{l.analyse_ia.diagnostic}</div>{l.analyse_ia.materiel&&<div style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}>{"Matériel: "+(l.analyse_ia.materiel||[]).join(", ")}</div>}</div>}
