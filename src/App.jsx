...
const isMobile=useIsMobile();
  ...
  async function register(data) {
    ...
    try {
      ...
      if (data.role === "pro") {
        fetch("https://www.click-fix.fr/api/send-email", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ type:"welcome_pro", to:data.email, data:{ prenom:result.prenom, email:data.email, entreprise:data.entreprise||"" } }) }).catch(console.log);
      }
...
setBusy(false);
  ...
  async function submitLead(formData) {
    ...
    try {
      ...
      fetch("https://www.click-fix.fr/api/save-lead",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(newLead)}).catch(console.log);
...
<section style={{padding:(window.innerWidth<=900)?'60px 20px':'100px 48px',background:'#1d1d1f',borderTop:'0.5px solid rgba(255,255,255,0.06)'}}>
  <div style={{maxWidth:1080,margin:'0 auto'}}>
    <div style={{marginBottom:56,textAlign:'center'}}>
      <h2 style={{fontSize:'clamp(28px,3.5vw,48px)',fontWeight:800,letterSpacing:'-1.8px',marginBottom:8,color:'#fff'}}>Click&amp;fix en chiffres</h2>
      <p style={{fontSize:15,color:'rgba(255,255,255,0.4)',fontWeight:400}}>La confiance de milliers de particuliers et d&apos;artisans en Île-de-France</p>
    </div>
        ...
        <div key={label} style={{textAlign:'center'}}>
          <div style={{width:140,height:140,borderRadius:'50%',background:'conic-gradient('+color+' '+(pct*3.6)+'deg, rgba(255,255,255,0.08) 0deg)',margin:'0 auto 16px',display:'flex',alignItems:'center',justifyContent:'center'}}>
            <div style={{width:106,height:106,borderRadius:'50%',background:'#1d1d1f',display:'flex',alignItems:'center',justifyContent:'center'}}>
              <span style={{fontSize:26,fontWeight:800,color:'#fff',letterSpacing:'-1px'}}>{pct}%</span>
            </div>
          </div>
          <div style={{fontSize:14,color:'rgba(255,255,255,0.55)',fontWeight:500}}>{label}</div>
        </div>
      ))}
    </div>

    <div style={{textAlign:'center',marginBottom:32}}>
      <div style={{fontSize:13,fontWeight:700,color:'rgba(255,255,255,0.4)',letterSpacing:1.5,textTransform:'uppercase'}}>Taux de pénétration par métier en Île-de-France</div>
        ...
        <div key={label} style={{textAlign:'center'}}>
          <div style={{width:88,height:88,borderRadius:'50%',background:'conic-gradient('+color+' '+(pct*3.6)+'deg, rgba(255,255,255,0.08) 0deg)',margin:'0 auto 10px',display:'flex',alignItems:'center',justifyContent:'center'}}>
            <div style={{width:66,height:66,borderRadius:'50%',background:'#1d1d1f',display:'flex',alignItems:'center',justifyContent:'center'}}>
              <span style={{fontSize:15,fontWeight:800,color:'#fff'}}>{pct}%</span>
            </div>
          </div>
    ...
    <div style={{display:'flex',flexDirection:(window.innerWidth<=900)?'column':'row',alignItems:'center',justifyContent:'center',gap:32,marginBottom:64}}>
      {(()=>{
        ...
        return(
          <>
            <div style={{width:150,height:150,borderRadius:'50%',background:'conic-gradient('+gradient+')',flexShrink:0}}/>
            <div>
              <div style={{fontSize:13,fontWeight:700,color:'rgba(255,255,255,0.55)',marginBottom:12,letterSpacing:0.5,textTransform:'uppercase'}}>Part de marché en Île-de-France</div>
              <div style={{display:'flex',flexDirection:'column',gap:8}}>
                {slices.map(([label,val,color])=>(
                  <div key={label} style={{display:'flex',alignItems:'center',gap:9,fontSize:13.5,color:'rgba(255,255,255,0.5)'}}>
...
const dest=lead.lat&&lead.lon?lead.lat+","+lead.lon:encodeURIComponent((lead.adresse||"")+" "+(lead.ville||"")+" France");
useEffect(()=>{
navigator.geolocation.getCurrentPosition(pos=>{setMyPos({lat:pos.coords.latitude,lon:pos.coords.longitude});},err=>console.warn(err),{enableHighAccuracy:true});
},[]);
useEffect(()=>{
const clat=parseFloat(lead.lat)||48.8566;const clon=parseFloat(lead.lon)||2.3522;
const init=()=>{
const el=document.getElementById(mapId);if(!el||!window.L)return;
if(mapRef.current){mapRef.current.remove();}
const map=window.L.map(el,{zoomControl:false,attributionControl:false}).setView([clat,clon],14);
mapRef.current=map;
window.L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png").addTo(map);
const clientHtml="<div style='width:14px;height:14px;background:#38bdf8;border-radius:50%;border:2.5px solid #fff;box-shadow:0 0 0 4px rgba(56,189,248,0.3)'></div>";
window.L.marker([clat,clon],{icon:window.L.divIcon({html:clientHtml,iconSize:[14,14],className:""})}).addTo(map).bindPopup("Client");
if(myPos){
const artHtml="<div style='position:relative;width:36px;height:36px'><div style='position:absolute;inset:0;background:rgba(255,111,0,0.3);border-radius:50%;animation:pulse 1.5s infinite'></div><div style='position:absolute;inset:4px;background:#FF6F00;border-radius:50%;border:2px solid #fff;display:flex;align-items:center;justify-content:center;font-size:14px'>🔧</div></div>";
window.L.marker([myPos.lat,myPos.lon],{icon:window.L.divIcon({html:artHtml,iconSize:[36,36],iconAnchor:[18,18],className:""})}).addTo(map).bindPopup("Vous");
window.L.polyline([[myPos.lat,myPos.lon],[clat,clon]],{color:"#FF6F00",weight:3,opacity:0.7,dashArray:"8,6"}).addTo(map);
map.fitBounds([[myPos.lat,myPos.lon],[clat,clon]],{padding:[40,40]});
}
};
const css=document.getElementById("leaflet-css");
if(!css){const c=document.createElement("link");c.id="leaflet-css";c.rel="stylesheet";c.href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css";document.head.appendChild(c);}
if(window.L){init();}else{const s=document.createElement("script");s.src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js";s.onload=init;document.head.appendChild(s);}
return()=>{if(mapRef.current){mapRef.current.remove();mapRef.current=null;}};
},[myPos,lead.lat,lead.lon]);
return(<div style={{marginTop:12,borderRadius:16,overflow:"hidden",background:"#1a1a2e"}}>
<style>{"@keyframes pulse{0%,100%{transform:scale(1);opacity:0.6}50%{transform:scale(1.4);opacity:0.2}}"}</style>
<div style={{padding:"10px 14px",background:"#1d1d1f",display:"flex",gap:8}}>
<a href={lead.lat&&lead.lon?"https://waze.com/ul?ll="+lead.lat+","+lead.lon+"&navigate=yes":"https://waze.com/ul?q="+dest} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} style={{flex:1,padding:"9px",background:"rgba(0,119,195,0.15)",border:"1px solid rgba(0,119,195,0.3)",borderRadius:10,color:"#38bdf8",fontWeight:700,fontSize:12,textDecoration:"none",textAlign:"center",display:"block"}}>🗺 Waze</a>
<a href={lead.lat&&lead.lon?"https://maps.google.com/maps?daddr="+lead.lat+","+lead.lon:"https://maps.google.com/maps?daddr="+dest} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} style={{flex:1,padding:"9px",background:"rgba(66,133,244,0.15)",border:"1px solid rgba(66,133,244,0.3)",borderRadius:10,color:"#4285f4",fontWeight:700,fontSize:12,textDecoration:"none",textAlign:"center",display:"block"}}>📍 Google Maps</a>
</div>
<div id={mapId} style={{height:220,width:"100%"}}/>
</div>);
}
...
function RatingBlock({l,ctx}){
  ...
  async function envoyerNote(){
    if(note===0)return;
    const AK="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpcHF0cWV6bnR6Y214d2lhcWR6Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MDA3OTkxMCwiZXhwIjoyMDk1NjU1OTEwfQ.NJxvcp7MJEGbpNmvjkwDGc4CJCswcoLZdGUSw0EDisU";
    await fetch("https://bipqtqezntzcmxwiaqdz.supabase.co/rest/v1/leads?id=eq."+l.id,{method:"PATCH",headers:{"Content-Type":"application/json","apikey":AK,"Authorization":"Bearer "+AK},body:JSON.stringify({note_client:note,commentaire_client:commentaire})});
    setSent(true);
    ctx.notify("Merci pour votre avis !");
  }
  if(sent)return <div style={{marginTop:10,fontSize:12,color:"#22c55e"}}>Merci, votre avis a ete envoye !</div>;
  return(
    <div style={{marginTop:12,padding:"12px",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10}} onClick={e=>e.stopPropagation()}>
      <div style={{fontSize:12,fontWeight:700,color:"rgba(255,255,255,0.6)",marginBottom:8}}>Notez cette intervention</div>
      <div style={{display:"flex",gap:4,marginBottom:8}}>
        {[1,2,3,4,5].map(n=>(
          <span key={n} onClick={()=>setNote(n)} onMouseEnter={()=>setHover(n)} onMouseLeave={()=>setHover(0)} style={{fontSize:24,cursor:"pointer",color:(hover||note)>=n?"#FBC005":"rgba(255,255,255,0.15)"}}>★</span>
        ))}
      </div>
      <textarea value={commentaire} onChange={e=>setCommentaire(e.target.value)} placeholder="Un commentaire (facultatif)" style={{width:"100%",minHeight:60,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,color:"#fff",fontSize:13,padding:8,fontFamily:"inherit",resize:"none",boxSizing:"border-box"}}/>
      <button onClick={envoyerNote} disabled={note===0} style={{marginTop:8,width:"100%",padding:10,background:note===0?"rgba(255,255,255,0.06)":"#FBC005",border:"none",borderRadius:8,color:note===0?"rgba(255,255,255,0.3)":"#000",fontWeight:700,fontSize:13,cursor:note===0?"default":"pointer"}}>Envoyer mon avis</button>
    </div>
  );
}
