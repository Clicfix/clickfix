...
const [showRecap,setShowRecap]=useState(false);const [showAddr,setShowAddr]=useState(false);
...
async function confirmAndSend(){await ctx.submitLead({...leadData,...addrForm,prenom:s?.prenom,nom:s?.nom,email:s?.email,tel:s?.tel,creneaux:slots,type:leadData.travaux,nb_artisans:leadData.nb_artisans||3,details:leadData.details||"",message:""});setDone(true);}return(
...
{showCal&&<div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(56,189,248,0.2)",borderRadius:16,padding:16,marginTop:8}}><div style={{color:"#38bdf8",fontWeight:700,fontSize:14,marginBottom:12}}>Vos disponibilites ({slots.length} creneau{slots.length>1?"x":""} — minimum 3)</div><div style={{overflowX:"auto"}}><table style={{borderCollapse:"collapse",fontSize:12,color:"#fff",minWidth:400}}><thead><tr><th style={{padding:"6px 8px",color:"rgba(255,255,255,0.4)"}}></th>{hours.map(h=><th key={h} style={{padding:"6px 8px",color:"rgba(255,255,255,0.4)",fontWeight:600}}>{h}</th>)}</tr></thead><tbody>{days.map(d=>{const dateStr=d.toLocaleDateString("fr-FR");const dayName=d.toLocaleDateString("fr-FR",{weekday:"short"});return(<tr key={dateStr}><td style={{padding:"6px 8px",color:"rgba(255,255,255,0.5)",whiteSpace:"nowrap",paddingRight:12}}>{dayName} {dateStr}</td>{hours.map(h=>{const key=dateStr+"_"+h;const sel=slots.find(sl=>sl.key===key);return(<td key={h} style={{padding:"4px"}}><div onClick={()=>toggleSlot(dateStr,h)} style={{width:28,height:28,borderRadius:6,background:sel?"#38bdf8":"rgba(255,255,255,0.05)",border:"1px solid "+(sel?"#38bdf8":"rgba(255,255,255,0.1)"),cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:sel?"#07090f":"transparent",margin:"0 auto"}}>{sel?"✓":""}</div></td>);})}</tr>)})}</tbody></table></div><BigBtn style={{marginTop:14,background:slots.length>=3?"linear-gradient(135deg,#38bdf8,#0ea5e9)":"rgba(255,255,255,0.1)"}} onClick={submitWithSlots}>Envoyer ma demande ({slots.length} creneau{slots.length>1?"x":""})</BigBtn></div>}
{showRecap&&<div style={{background:"rgba(56,189,248,0.05)",border:"1px solid rgba(56,189,248,0.2)",borderRadius:16,padding:20,marginTop:8}}><div style={{color:"#38bdf8",fontWeight:700,fontSize:16,marginBottom:16}}>Recapitulatif de votre demande</div><div style={{display:"grid",gap:10,marginBottom:16}}>{[["Travaux",leadData?.travaux],["Specialite",leadData?.precision],["Details",leadData?.details],["Surface",leadData?.surface],["Budget",leadData?.budget],["Adresse",addrForm.adresse+", "+addrForm.ville+" "+addrForm.code_postal],["Artisans",leadData?.nb_artisans+" artisans"],["Creneaux",slots.length+" selectionne(s)"]].map(([k,v])=>v&&<div key={k} style={{display:"flex",gap:8,padding:"8px 12px",background:"rgba(255,255,255,0.04)",borderRadius:8}}><span style={{color:"rgba(255,255,255,0.4)",fontSize:12,minWidth:80}}>{k}</span><span style={{color:"#fff",fontSize:13,fontWeight:500}}>{v}</span></div>)}</div><div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:16}}>{slots.map(sl=><span key={sl.key} style={{fontSize:11,padding:"4px 10px",borderRadius:6,background:"rgba(56,189,248,0.1)",color:"#38bdf8",border:"1px solid rgba(56,189,248,0.2)"}}>{sl.label}</span>)}</div><div style={{display:"flex",gap:10}}><button onClick={()=>{setShowRecap(false);setShowCal(true);}} style={{flex:1,padding:"11px",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,color:"rgba(255,255,255,0.5)",cursor:"pointer",fontSize:13}}>Modifier</button><BigBtn style={{flex:2,background:"linear-gradient(135deg,#38bdf8,#0ea5e9)"}} onClick={confirmAndSend}>Confirmer et envoyer</BigBtn></div></div>}</div>
