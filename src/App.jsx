...
                    ))}
                    ...
                    {l.paiement_statut==="paye"&&<div style={{marginTop:12,padding:"12px",background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:10}}><div style={{fontSize:13,fontWeight:700,color:"#22c55e",marginBottom:8}}>✅ Intervention payée — {l.prix_final}EUR</div><button onClick={e=>{e.stopPropagation();window.location.href="/api/generate-invoice?lead_id="+l.id+"&prix_final="+(l.prix_final||"")+"&travaux="+(l.travaux||"")+"&client_nom="+(l.client_nom||"")+"&date="+new Date(l.created_at).toLocaleDateString("fr-FR");}} style={{width:"100%",padding:"10px",background:"rgba(34,197,94,0.15)",border:"1px solid rgba(34,197,94,0.3)",borderRadius:10,color:"#22c55e",fontWeight:700,fontSize:13,cursor:"pointer"}}>📄 Voir la facture</button></div>}
                  </div>
                )}
              </div>
            ))}
