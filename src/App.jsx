...
async function send(){if(!input.trim()||busy)return;const newMsgs=[...msgs,{role:"user",content:input}];setMsgs(newMsgs);setInput("");setBusy(true);try{const r=await fetch("/api/ai-chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:newMsgs,prenom:s?.prenom})});const d=await r.json();const text=d.text||"";const tag="LEAD";const re=new RegExp("<"+tag+">(.*?)<\/"+tag+">","s");const m2=text.match(re);if(m2){try{const lead=JSON.parse(m2[1]);setLeadData(lead);setMsgs(prev=>[...prev,{role:"assistant",content:"Parfait ! Entrez l adresse exacte du chantier."}]);setShowAddr(true);}catch(e){setMsgs(prev=>[...prev,{role:"assistant",content:text.replace(re,"").trim()}]);}}else{setMsgs(prev=>[...prev,{role:"assistant",content:text}]);}}catch(e){setMsgs(prev=>[...prev,{role:"assistant",content:"Erreur. Reessayez."}]);}setBusy(false);}return(
