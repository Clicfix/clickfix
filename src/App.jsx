...
                    ))}
                    ...
                    {l.paiement_statut==="paye"&&<div style={{marginTop:12,padding:"12px",background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:10}}><div style={{fontSize:13,fontWeight:700,color:"#22c55e",marginBottom:8}}>✅ Intervention payée — {l.prix_final}EUR</div><button onClick={e=>{e.stopPropagation();const showInvoice=()=>{const doc=new window.jspdf.jsPDF();const prix=parseFloat(l.prix_final)||0;const ht=prix;doc.setFillColor(29,29,31);doc.rect(0,0,210,28,"F");doc.setFont("helvetica","bold");doc.setFontSize(16);doc.setTextColor(255,255,255);doc.text("Click&fix",20,16);doc.setFontSize(8);doc.setFont("helvetica","normal");doc.setTextColor(180,180,180);doc.text("Services a domicile",20,23);doc.setFont("helvetica","bold");doc.setFontSize(14);doc.setTextColor(255,255,255);doc.text("FACTURE",190,16,{align:"right"});doc.setFont("helvetica","normal");doc.setFontSize(9);doc.setTextColor(180,180,180);doc.text("N° CF-"+new Date().getFullYear()+"-"+String(l.id).padStart(4,"0"),190,23,{align:"right"});doc.setFontSize(9);doc.setTextColor(100,100,100);doc.text("Date d emission: "+new Date(l.created_at).toLocaleDateString("fr-FR"),20,40);doc.text("Lieu d intervention: France",20,47);doc.setDrawColor(230,230,230);doc.line(20,53,190,53);doc.setFillColor(248,248,250);doc.roundedRect(20,57,82,38,2,2,"F");doc.roundedRect(108,57,82,38,2,2,"F");doc.setFont("helvetica","bold");doc.setFontSize(8);doc.setTextColor(120,120,120);doc.text("CLIENT",25,64);doc.text("ARTISAN",113,64);doc.setFont("helvetica","bold");doc.setFontSize(10);doc.setTextColor(20,20,20);doc.text(l.client_nom||"Client",25,72);doc.text(l.artisan_nom||"Artisan",113,72);doc.setFont("helvetica","normal");doc.setFontSize(8);doc.setTextColor(100,100,100);if(l.client_email)doc.text(l.client_email,25,79);doc.text("Partenaire Click&fix",113,79);if(l.client_tel)doc.text(l.client_tel,25,86);doc.setDrawColor(230,230,230);doc.line(20,102,190,102);doc.setFillColor(29,29,31);doc.rect(20,106,170,9,"F");doc.setFont("helvetica","bold");doc.setFontSize(8);doc.setTextColor(255,255,255);doc.text("DESCRIPTION",25,112);doc.text("QTE",138,112);doc.text("MONTANT",185,112,{align:"right"});doc.setFillColor(252,252,252);doc.rect(20,115,170,18,"F");doc.setFont("helvetica","bold");doc.setFontSize(10);doc.setTextColor(20,20,20);const trav=l.travaux||"Intervention";doc.text(trav.charAt(0).toUpperCase()+trav.slice(1),25,124);doc.setFont("helvetica","normal");doc.setFontSize(8);doc.setTextColor(120,120,120);if(l.details){const det=doc.splitTextToSize(l.details,95);doc.text(det,25,130);}doc.setFont("helvetica","normal");doc.setFontSize(10);doc.setTextColor(20,20,20);doc.text("1",140,124);doc.text(prix.toFixed(2)+" EUR",185,124,{align:"right"});doc.setDrawColor(230,230,230);doc.line(20,136,190,136);doc.setFontSize(9);doc.setTextColor(80,80,80);doc.text("Sous-total HT:",130,145);doc.text(ht.toFixed(2)+" EUR",185,145,{align:"right"});doc.setFont("helvetica","italic");doc.setFontSize(8);doc.setTextColor(140,140,140);doc.text("TVA non applicable - Art. 293 B du CGI",130,153);doc.text("0.00 EUR",185,153,{align:"right"});doc.setDrawColor(200,200,200);doc.line(130,157,190,157);doc.setFillColor(29,29,31);doc.roundedRect(120,160,70,14,2,2,"F");doc.setFont("helvetica","bold");doc.setFontSize(11);doc.setTextColor(255,255,255);doc.text("Total TTC: "+prix.toFixed(2)+" EUR",155,169,{align:"center"});doc.setFont("helvetica","normal");doc.setFontSize(8);doc.setTextColor(150,150,150);doc.text("Ref. paiement: "+(l.payment_intent_id||"N/A"),20,185);doc.setDrawColor(200,200,200);doc.line(20,268,190,268);doc.setFontSize(7.5);doc.text("Click&fix — contact@click-fix.fr — www.click-fix.fr",105,273,{align:"center"});doc.text("Plateforme de mise en relation entre particuliers et artisans",105,279,{align:"center"});const url=doc.output("bloburl");const overlay=document.createElement("div");overlay.style.cssText="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;";const btn=document.createElement("button");btn.textContent="✕ Fermer";btn.style.cssText="margin-bottom:12px;padding:8px 20px;background:#fff;border:none;border-radius:8px;font-weight:700;cursor:pointer;font-size:14px;";btn.onclick=()=>document.body.removeChild(overlay);const iframe=document.createElement("iframe");iframe.src=url;iframe.style.cssText="width:90%;max-width:700px;height:80vh;border:none;border-radius:12px;";overlay.appendChild(btn);overlay.appendChild(iframe);document.body.appendChild(overlay);};if(window.jspdf){showInvoice();}else{const s=document.createElement("script");s.src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js";s.onload=showInvoice;document.head.appendChild(s);}}} style={{width:"100%",padding:"10px",background:"rgba(34,197,94,0.15)",border:"1px solid rgba(34,197,94,0.3)",borderRadius:10,color:"#22c55e",fontWeight:700,fontSize:13,cursor:"pointer"}}>📄 Voir la facture</button></div>}
                  </div>
                )}
              </div>
            ))}
