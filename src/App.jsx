...
                    ))}
                    ...
                    {l.paiement_statut==="paye"&&<div style={{marginTop:12,padding:"12px",background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:10}}><div style={{fontSize:13,fontWeight:700,color:"#22c55e",marginBottom:8}}>✅ Intervention payée — {l.prix_final}EUR</div><button onClick={e=>{e.stopPropagation();const showInvoice=()=>{const doc=new window.jspdf.jsPDF();doc.setFont("helvetica","bold");doc.setFontSize(22);doc.setTextColor(255,111,0);doc.text("Click&fix",20,20);doc.setTextColor(0,0,0);doc.setFontSize(18);doc.text("Facture CF-"+String(l.id).padStart(4,"0"),20,35);doc.setFontSize(11);doc.setFont("helvetica","normal");doc.setTextColor(120,120,120);doc.text("Date: "+new Date(l.created_at).toLocaleDateString("fr-FR"),20,48);doc.setDrawColor(255,111,0);doc.setLineWidth(0.5);doc.line(20,52,190,52);doc.setTextColor(0,0,0);doc.setFont("helvetica","bold");doc.text("Client:",20,65);doc.setFont("helvetica","normal");doc.text(l.client_nom||"Client",70,65);doc.setFont("helvetica","bold");doc.text("Artisan:",20,75);doc.setFont("helvetica","normal");doc.text(l.artisan_nom||"Artisan",70,75);doc.setFont("helvetica","bold");doc.text("Prestation:",20,85);doc.setFont("helvetica","normal");doc.text(l.travaux||"Intervention",70,85);doc.setFillColor(255,111,0);doc.roundedRect(20,110,170,20,3,3,"F");doc.setFont("helvetica","bold");doc.setFontSize(13);doc.setTextColor(255,255,255);doc.text("Total TTC: "+(l.prix_final||0)+"EUR",105,124,{align:"center"});doc.setFontSize(9);doc.setTextColor(150,150,150);doc.text("Click&fix — contact@click-fix.fr",105,275,{align:"center"});const url=doc.output("bloburl");const overlay=document.createElement("div");overlay.style.cssText="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;";const btn=document.createElement("button");btn.textContent="✕ Fermer";btn.style.cssText="margin-bottom:12px;padding:8px 20px;background:#fff;border:none;border-radius:8px;font-weight:700;cursor:pointer;font-size:14px;";btn.onclick=()=>document.body.removeChild(overlay);const iframe=document.createElement("iframe");iframe.src=url;iframe.style.cssText="width:90%;max-width:700px;height:80vh;border:none;border-radius:12px;";overlay.appendChild(btn);overlay.appendChild(iframe);document.body.appendChild(overlay);};if(window.jspdf){showInvoice();}else{const s=document.createElement("script");s.src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js";s.onload=showInvoice;document.head.appendChild(s);}}} style={{width:"100%",padding:"10px",background:"rgba(34,197,94,0.15)",border:"1px solid rgba(34,197,94,0.3)",borderRadius:10,color:"#22c55e",fontWeight:700,fontSize:13,cursor:"pointer"}}>📄 Voir la facture</button></div>}
                  </div>
                )}
              </div>
            ))}
