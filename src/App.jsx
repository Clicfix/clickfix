...
                    {l.assigned_to&&<ArtisanInfo id={l.assigned_to}/>}
                    {l.photo&&<a href={l.photo} target="_blank" rel="noopener noreferrer" style={{display:"block",marginTop:12,borderRadius:12,overflow:"hidden"}}><img src={l.photo} alt="Photo" style={{width:"100%",maxHeight:180,objectFit:"cover",borderRadius:12}}/><div style={{fontSize:11,color:"rgba(255,255,255,0.4)",textAlign:"center",marginTop:4}}>Voir la photo en grand →</div></a>}{l.analyse_ia&&l.analyse_ia.diagnostic&&<div style={{marginTop:12,padding:"12px",background:"rgba(34,197,94,0.05)",border:"1px solid rgba(34,197,94,0.15)",borderRadius:12}}><div style={{fontSize:10,fontWeight:700,color:"#22c55e",letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>Details de l intervention</div><div style={{fontSize:13,color:"rgba(255,255,255,0.8)",marginBottom:4}}>{l.analyse_ia.diagnostic}</div>{l.analyse_ia.materiel&&<div style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}>{"Matériel: "+(l.analyse_ia.materiel||[]).join(", ")}</div>}</div>}
