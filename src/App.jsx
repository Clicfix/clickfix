...
  useEffect(()=>{if(sess&&sess.id){const AK="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpcHF0cWV6bnR6Y214d2lhcWR6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAwNzk5MTAsImV4cCI6MjA5NTY1NTkxMH0.OmScmhwC-qOHf1tW81UxHgk0OHpSJvz5NCpktzMa81M";const filter=sess.role==="pro"?"assigned_to=eq."+sess.id:"client_email=eq."+encodeURIComponent(sess.email||"");fetch("https://bipqtqezntzcmxwiaqdz.supabase.co/rest/v1/leads?select=*&"+filter+"&order=created_at.desc",{headers:{"apikey":AK,"Authorization":"Bearer "+(sess.token||AK)}}).then(r=>r.json()).then(d=>{if(Array.isArray(d))setLeads(d);}).catch(()=>{});}},[sess&&sess.id]);
