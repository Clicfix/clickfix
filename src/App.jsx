...
  useEffect(()=>{if(sess&&sess.id){fetch("https://bipqtqezntzcmxwiaqdz.supabase.co/rest/v1/leads?select=id,statut,travaux,precision,details,adresse,ville,surface,budget,nb_artisans,creneaux,assigned_to,client_nom,client_tel,client_email,photo,analyse_ia,payment_intent_id,paiement_statut,prix_final,montant_pre_autorise,user_id,lat,lon,created_at&order=created_at.desc&limit=100",{headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpcHF0cWV6bnR6Y214d2lhcWR6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAwNzk5MTAsImV4cCI6MjA5NTY1NTkxMH0.OmScmhwC-qOHf1tW81UxHgk0OHpSJvz5NCpktzMa81M","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpcHF0cWV6bnR6Y214d2lhcWR6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAwNzk5MTAsImV4cCI6MjA5NTY1NTkxMH0.OmScmhwC-qOHf1tW81UxHgk0OHpSJvz5NCpktzMa81M"}}).then(r=>r.json()).then(d=>{if(Array.isArray(d))setLeads(d);}).catch(()=>{});}},[sess&&sess.id]);
  useEffect(()=>{if(!sess?.id)return;const iv=setInterval(()=>{fetch("https://bipqtqezntzcmxwiaqdz.supabase.co/rest/v1/leads?select=id,statut,travaux,precision,details,adresse,ville,surface,budget,nb_artisans,creneaux,assigned_to,client_nom,client_tel,client_email,photo,analyse_ia,payment_intent_id,paiement_statut,prix_final,montant_pre_autorise,user_id,lat,lon,created_at&order=created_at.desc&limit=100",{headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpcHF0cWV6bnR6Y214d2lhcWR6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAwNzk5MTAsImV4cCI6MjA5NTY1NTkxMH0.OmScmhwC-qOHf1tW81UxHgk0OHpSJvz5NCpktzMa81M","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpcHF0cWV6bnR6Y214d2lhcWR6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAwNzk5MTAsImV4cCI6MjA5NTY1NTkxMH0.OmScmhwC-qOHf1tW81UxHgk0OHpSJvz5NCpktzMa81M"}}).then(r=>r.json()).then(d=>{if(Array.isArray(d))setLeads(d);}).catch(()=>{});},5000);return()=>clearInterval(iv);},[sess?.id]);
