...
{l.artisan_statut==="arrive"&&l.paiement_statut==="pre_autorise"&&<div style={{marginTop:12,padding:"12px 16px",background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:12}}><div style={{fontSize:13,fontWeight:700,color:"#22c55e",marginBottom:10}}>✅ Votre artisan est arrivé — intervention en cours</div><button onClick={async e=>{e.stopPropagation();const AK="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpcHF0cWV6bnR6Y214d2lhcWR6Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MDA3OTkxMCwiZXhwIjoyMDk1NjU1OTEwfQ.NJxvcp7MJEGbpNmvjkwDGc4CJCswcoLZdGUSw0EDisU";await fetch("https://bipqtqezntzcmxwiaqdz.supabase.co/rest/v1/leads?id=eq."+l.id,{method:"PATCH",headers:{"Content-Type":"application/json","apikey":AK,"Authorization":"Bearer "+AK},body:JSON.stringify({artisan_statut:"termine"})});ctx.notify("Intervention marquee comme terminee");}} style={{width:"100%",padding:"10px",background:"#22c55e",border:"none",borderRadius:10,color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer"}}>✓ Intervention terminée</button></div>}
                  </div>
                )}
              </div>
            ))}
