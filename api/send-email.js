...
const ftr='<div style="background:#1a1a2e;padding:16px;text-align:center;border-top:1px solid rgba(255,255,255,0.08)"><p style="margin:0;color:rgba(255,255,255,0.4);font-size:11px;font-family:Arial">Click&fix | contact@click-fix.fr | click-fix.fr</p></div>';
const wrap=c=>`<div style="background:#07090f;padding:20px;font-family:Arial"><div style="max-width:600px;margin:0 auto;border-radius:12px;overflow:hidden;border:1px solid rgba(255,255,255,0.08)">${hdr}<div style="background:#0d0f1a;padding:24px">${c}</div>${ftr}</div></div>`;
const row=(l,v)=>`<table width="100%" style="border-bottom:1px solid rgba(255,255,255,0.06);margin-bottom:6px"><tr><td style="color:rgba(255,255,255,0.45);font-size:13px;padding:6px 0;font-family:Arial">${l} :</td><td align="right" style="color:#fff;font-size:13px;font-weight:700;font-family:Arial;padding:6px 0">${v}</td></tr></table>`;
...
html=wrap(`<h2 style="color:#FF6F00;margin:0 0 12px;font-size:22px">Bienvenue ${data.prenom} !</h2><p style="color:rgba(255,255,255,0.7);font-size:14px;line-height:1.6;margin:0 0 16px">Votre compte artisan Click&fix est actif. Vous allez bientot recevoir des RDV qualifies.</p>${box(row("Prenom",data.prenom)+row("Email",data.email)+row("Entreprise",data.entreprise||"-")+row("Statut","Actif"))}<p style="color:#FBC005;font-size:13px;margin:0 0 8px;font-weight:700">Prochaines etapes :</p><ul style="color:rgba(255,255,255,0.6);font-size:13px;line-height:1.8;padding-left:20px;margin:0 0 16px"><li>Uploadez vos documents (Kbis, Decennale, RC Pro)</li><li>Choisissez votre pack de RDV</li><li>Recevez vos premiers clients</li></ul>${btn("Acceder a mon espace","https://click-fix.fr")}`);
}
else if(type==="new_lead_pro"){
subject="Nouveau RDV qualifie - Confirmez votre presence";
html=wrap(`<div style="background:rgba(255,111,0,0.1);border:1px solid rgba(255,111,0,0.3);border-radius:8px;padding:12px 16px;margin-bottom:16px"><p style="margin:0;color:#FF6F00;font-weight:700">Nouveau RDV qualifie disponible !</p></div><h2 style="color:#fff;margin:0 0 12px;font-size:20px">Bonjour ${data.prenom},</h2><p style="color:rgba(255,255,255,0.6);font-size:13px;margin:0 0 12px">Un client recherche un artisan :</p>${box(row("Travaux",data.travaux)+row("Surface",data.surface||"-")+row("Budget",data.budget||"-")+row("Ville",data.ville||"-")+row("Votre creneau",data.creneau||"Sur RDV"))}${btn("Confirmer ma presence","https://click-fix.fr","#22c55e")}`);
}
else if(type==="confirm_rdv_client"){
subject="Votre RDV est confirme - Coordonnees de votre artisan";
html=wrap(`<div style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.3);border-radius:8px;padding:12px 16px;margin-bottom:16px"><p style="margin:0;color:#22c55e;font-weight:700">Votre RDV est confirme !</p></div><h2 style="color:#fff;margin:0 0 12px;font-size:20px">Bonjour ${data.client_prenom},</h2><p style="color:rgba(255,255,255,0.6);font-size:13px;margin:0 0 12px">Voici les coordonnees de votre artisan :</p>${box(row("Artisan",data.artisan_prenom+" "+data.artisan_nom)+row("Entreprise",data.artisan_entreprise||"-")+row("Telephone",data.artisan_tel||"-")+row("Email",data.artisan_email||"-")+row("Creneau",data.creneau||"-"))}${btn("Voir mon espace","https://click-fix.fr")}`);
}
